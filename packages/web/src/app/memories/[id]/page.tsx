...
export default function MemoryDetailPage() {
  ...
  async function loadRelations(id: string) {
    try {
      ...
      const data = await api.getMemoryRelations(id, { includeAutoGenerated: true });
      setRelations({
        total: data.total,
        autoGenerated: data.autoGeneratedCount,
        manual: data.total - data.autoGeneratedCount,
          ...
          <div className="space-y-6">
            ...
            <div className="glassCard p-6">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-lg font-heading font-semibold">关联记忆</h2>
                {relations && relations.autoGenerated > 0 && (
                  <span className="flex items-center gap-1 text-xs text-emerald-400">
                    <Sparkles className="w-3 h-3" />
                    {relations.autoGenerated} 自动关联
                  </span>
                )}
              </div>
              ...
              {loadingRelations ? (
                ...
                <div className="space-y-3">
                  {relations.items.slice(0, 5).map((rel) => {
                    const relIcon = typeIcons[rel.targetMemory?.type || ''] || Brain;
                    const relColor = typeColors[rel.targetMemory?.type || ''] || '#6B7280';
                    const RelIcon = relIcon;
                    return (
                      <Link
                        key={rel.targetId}
                        href={`/memories/${rel.targetId}`}
                        className="block p-3 rounded-lg bg-background/30 border border-border/30 hover:border-cyan-500/30 hover:bg-cyan-500/5 transition-all cursor-pointer group"
                      >
                        <div className="flex items-start gap-2">
                          <div
                            className="p-1.5 rounded-md shrink-0"
                            style={{
                              background: `linear-gradient(135deg, ${relColor}20, ${relColor}10)`,
                              border: `1px solid ${relColor}30`,
                            }}
                          >
                            <RelIcon className="w-3 h-3" style={{ color: relColor }} />
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="text-sm font-medium truncate group-hover:text-cyan-400 transition-colors">
                              {rel.targetMemory?.summary || '未知记忆'}
                            </div>
                            <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
                              <span style={{ color: relColor }}>
                                {getMemoryTypeName(rel.targetMemory?.type || '')}
                              </span>
                              <span>•</span>
