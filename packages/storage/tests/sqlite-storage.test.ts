...
describe('SQLiteStorage', () => {
  ...
  describe('recall()', () => {
    ...
    it('应该支持按项目过滤', async () => {
      ...
      expect(result.memories.every((m) => m.meta.projectId === 'test-project')).toBe(true);
    });
    ...
    it('应该返回性能指标', async () => {
      ...
      expect(result.metrics?.strategyTime).toBeGreaterThanOrEqual(0);

      // 验证总时间大于等于各部分时间之和
      const totalMetrics =
        (result.metrics?.dbTime || 0) +
        (result.metrics?.parseTime || 0) +
        (result.metrics?.strategyTime || 0);
      expect(result.took).toBeGreaterThanOrEqual(totalMetrics - 1); // 允许1ms误差
    });
  });
  ...
  describe('FTS5全文搜索', () => {
    beforeEach(async () => {
      await storage.store({
        content: 'React性能优化：使用useMemo缓存计算结果',
        rawContext: {},
        projectId: 'test-project',
      });
    ...
    it('应该正确执行FTS5搜索', async () => {
      ...
      expect(result.memories.length).toBe(1);
      expect(result.memories[0].content.summary).toContain('React');
    });

    it('FTS5触发器应该自动同步数据', async () => {
      // 插入记忆后立即搜索应该能找到
      const { id } = await storage.store({
        content: 'TypeScript泛型最佳实践',
        rawContext: {},
        projectId: 'test-project',
      });
      ...
      expect(result.memories.length).toBe(1);
      expect(result.memories[0].meta.id).toBe(id);
    });
  });
});
