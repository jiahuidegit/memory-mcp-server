...
export class SQLiteStorage implements IStorage {
  ...
  async store(params: {
    ...
    // 同步写入 memory_relations 表（手动指定的关系）
    if (params.relations) {
      ...
      if (params.relations.relatedTo) {
        for (const targetId of params.relations.relatedTo) {
          relStmt.run(`rel_${nanoid()}`, id, targetId, 'relatedTo', 1.0, 0, null, timestamp);
        }
      }
      if (params.relations.replaces) {
        for (const targetId of params.relations.replaces) {
          relStmt.run(`rel_${nanoid()}`, id, targetId, 'replaces', 1.0, 0, null, timestamp);
        }
      }
      if (params.relations.impacts) {
        for (const targetId of params.relations.impacts) {
          relStmt.run(`rel_${nanoid()}`, id, targetId, 'impacts', 1.0, 0, null, timestamp);
        }
      }
   ...
   * 扩展关联记忆
   *
   * 从 memory_relations 表和 Memory 内嵌字段两处获取关联记忆
   * 自动获取关联的记忆，去重后返回
   */
  private expandRelatedMemories(
    ...
    // 1. 从 memory_relations 表获取关系
    if (memoryIds.length > 0) {
      const placeholders = memoryIds.map(() => '?').join(', ');
      const relSql = `SELECT * FROM memory_relations WHERE sourceId IN (${placeholders}) OR targetId IN (${placeholders})`;
      const relStmt = this.db.prepare(relSql);
      const relations = relStmt.all(...memoryIds, ...memoryIds) as any[];
